...
int						temp_periods = 10;		//number of periods to read temp
...
int32_t fast_temp_read (void){
	...
	return temp; //*1000 to convert ms to microsecond
}
...
void check_ClimbSensors(){ //Called in all Climb_State
	...
	if(temp_on){
		...
		tempvalue = fast_temp_read(); //gives 10* temperature in degree C
		if (tempvalue > TEMP_THRESHOLD && temp_flag == 0 && t2>=t1){ //maybe can use interrupt?
...
//			printf("State changed to Initialization\n");
			TEMP_THRESHOLD = fast_temp_read() + 50; //0.5 degrees C above current temp
