...
static void init_everything(){
    ...
    LPC_GPIOINT->IO2IntEnF |= 1<<10; //sw3 connected to BL_EN
...
int32_t fast_temp_read (void){
	int32_t time_diff = abs(t2-t1);
	//10T(C) = (period (us) / scalar_div10) - 2731 K
	return (time_diff*1000 / (10*TEMP_SCALAR_DIV10)) - 2731; //*1000 to convert ms to microsecond
}
...
void EINT0_IRQHandler(void){
	// SW3
	if ((LPC_GPIOINT->IO2IntStatF>>10)& 0x1){ //sw3 P2.10 with BL_EN
...
//			printf("State changed to Initialization\n");
			TEMP_THRESHOLD = temp_read() + 50; //0.5 degrees C above current temp
...
void EINT3_IRQHandler(void){
	...
	if ((LPC_GPIOINT ->IO0IntStatR>>2) & 0x1){ //temperature sensor
		...
		temp_periods++;
		if (temp_periods == 1) 			t1 = Get_Time(); 	//time at the start of 10 periods
		else if (temp_periods == 11)	t2 = Get_Time();	//time at the end of 10 periods
		//wait for awhile before resetting temp_periods for next reading
		else if (temp_periods == 20) 	temp_periods = 0;
	}
